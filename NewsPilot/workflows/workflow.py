...
from NewsPilot.utils.env import env_flag
...
class NewsPilotState(TypedDict, total=False):
...
def _node_init(state: NewsPilotState) -> NewsPilotState:
    analyzer = NewsAnalyzer()
    analyzer._initialize_and_check_config()
    return {"analyzer": analyzer}


def _node_crawl_news(state: NewsPilotState) -> NewsPilotState:
...
def _node_crawl_rss(state: NewsPilotState) -> NewsPilotState:
    analyzer = state["analyzer"]
...
def _node_llm_enrich(state: NewsPilotState) -> NewsPilotState:
    analyzer = state["analyzer"]

    if env_flag("NP_SKIP_LLM"):
        print("[LLM] NP_SKIP_LLM set, skipping LLM enrichment (agent-native mode)")
        return {"llm_enrichment": None, "llm_debug": {"skipped_reason": "skip_llm_env_var"}}
    ...
    try:
        from NewsPilot.llm.enrich import enrich_rss_and_persist
...
def _node_execute(state: NewsPilotState) -> NewsPilotState:
    analyzer = state["analyzer"]
...
def _node_finalize(state: NewsPilotState) -> NewsPilotState:
...
def build_and_run(*, include_news: bool, include_rss: bool, llm_tasks: List[str]) -> Dict[str, Any]:
    ...
    graph = StateGraph(NewsPilotState)
