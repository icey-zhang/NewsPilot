...
from NewsPilot.core.frequency import matches_word_groups, _word_matches
from NewsPilot.utils.env import env_flag, env_int
...
def count_rss_frequency(
    ...
    """
    from NewsPilot.utils.time import format_iso_time_friendly
    from NewsPilot.core.frequency import explain_word_group_match
    ...
    debug_filter = env_flag("NP_RSS_DEBUG_FILTER")
    debug_filter_max = env_int("NP_RSS_DEBUG_FILTER_MAX", default=30)
    skipped_debug: List[Dict[str, str]] = []
    ...
    for item in rss_items:
        ...
        if not matches_word_groups(title, word_groups, filter_words, global_filters):
            if debug_filter and len(skipped_debug) < debug_filter_max:
                info = explain_word_group_match(title, word_groups, filter_words, global_filters)
                skipped_debug.append(
                    {
                        "reason": info.get("reason", ""),
                        "detail": info.get("detail", ""),
                        "feed": str(item.get("feed_name", item.get("feed_id", "")) or ""),
                        "title": str(title or ""),
                        "url": str(url or ""),
                    }
                )
    ...
    if not quiet:
        ...
        if debug_filter:
            skipped_count = total_items - matched_count
            print(f"[RSS][DEBUG] 未匹配/被过滤：{skipped_count} 条（最多展示 {debug_filter_max} 条）")
            for i, row in enumerate(skipped_debug, 1):
                ...
                prefix = f"[{i}] {reason}{tail}"
                if feed:
                    prefix += f" | {feed}"
                print(f"[RSS][DEBUG] {prefix} | {t} | {u}")
