...
def explain_word_group_match(
    ...
    """
    Explain why a title matched or was filtered out by word group rules.

    Returns:
        {
          "matched": "true|false",
          "reason": "matched|empty_title|global_filter|filter_word|no_group_match",
          "detail": "<optional details such as matched group/filter word>"
        }
    """
    if not isinstance(title, str):
        title = str(title) if title is not None else ""
    if not title.strip():
        return {"matched": "false", "reason": "empty_title", "detail": ""}

    title_lower = title.lower()

    if global_filters:
        for global_word in global_filters:
            w = (global_word or "").strip()
            if w and w.lower() in title_lower:
                return {"matched": "false", "reason": "global_filter", "detail": w}

    for filter_item in filter_words or []:
        if _word_matches(filter_item, title_lower):
            if isinstance(filter_item, dict):
                d = (filter_item.get("word") or "").strip()
            else:
                d = str(filter_item).strip()
            return {"matched": "false", "reason": "filter_word", "detail": d}

    if not word_groups:
        return {"matched": "true", "reason": "matched", "detail": "no_word_groups"}
    ...
    for group in word_groups:
        ...
        group_key = str(group.get("display_name") or group.get("group_key") or "").strip()
        return {"matched": "true", "reason": "matched", "detail": group_key or ""}

    return {"matched": "false", "reason": "no_group_match", "detail": ""}
