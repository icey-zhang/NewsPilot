...
def _load_report_config(config_data: Dict) -> Dict:
    ...
    show_rss_new_env = _get_env_bool("SHOW_RSS_NEW_SECTION")
    hide_rss_without_llm_env = _get_env_bool("HIDE_RSS_WITHOUT_LLM")
    ...
    return {
        ...
        "SHOW_RSS_NEW_SECTION": show_rss_new_env if show_rss_new_env is not None else report_config.get("show_rss_new_section", True),
        "HIDE_RSS_WITHOUT_LLM": hide_rss_without_llm_env if hide_rss_without_llm_env is not None else report_config.get("hide_rss_without_llm", False),
    }
...
def _load_llm_config(config_data: Dict) -> Dict:
    """加载 LLM 富化配置（OpenAI-compatible）"""
    llm = config_data.get("llm", {}) or {}
    tasks_raw = llm.get("tasks", [])
    tasks: List[str] = []
    if isinstance(tasks_raw, list):
        tasks = [str(x).strip().lower() for x in tasks_raw if str(x).strip()]
    elif isinstance(tasks_raw, str):
        # 兼容字符串形式：summary,classify
        parts = tasks_raw.replace(";", ",").replace("|", ",").split(",")
        tasks = [p.strip().lower() for p in parts if p.strip()]

    fulltext = llm.get("fulltext", {}) or {}
    item_enrich = llm.get("item_enrich", {}) or {}
    ...
    return {
        ...
        "TIMEOUT": int(llm.get("timeout", 90) or 90),
        "TASKS": tasks,
        "ITEM_ENRICH": {
            # bulk: one request with many items (default, cheaper, but can timeout)
            # single: one request per item (more requests, less timeout-prone)
            "MODE": (str(item_enrich.get("mode", "bulk")) or "bulk").strip().lower(),
            # 0 = use default cap (newest 50); set a positive number to enrich more items
            "MAX_ITEMS": _to_int(item_enrich.get("max_items", 0), 0),
            # optional sleep between item requests in single mode
            "REQUEST_INTERVAL_MS": _to_int(item_enrich.get("request_interval_ms", 0), 0),
        },
        "FULLTEXT": {
            "ENABLED": bool(fulltext.get("enabled", False)),
            "TIMEOUT": _to_int(fulltext.get("timeout", 15), 15),
            "MAX_CHARS": _to_int(fulltext.get("max_chars", 6000), 6000),
            "MAX_BYTES": _to_int(fulltext.get("max_bytes", 1200000), 1200000),
            "MIN_PARAGRAPH_CHARS": _to_int(fulltext.get("min_paragraph_chars", 60), 60),
            "REQUEST_INTERVAL_MS": _to_int(fulltext.get("request_interval_ms", 200), 200),
            "USE_PROXY": bool(fulltext.get("use_proxy", False)),
            "PROXY_URL": (fulltext.get("proxy_url", "") or "").strip(),
            "MAX_ITEMS": _to_int(fulltext.get("max_items", 10), 10),
        },
    }
...
def load_config(config_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not isinstance(platforms, list):
        print(f"[警告] platforms 配置应为列表，当前为 {type(platforms).__name__}，将忽略并使用空列表")
        platforms = []
