# coding=utf-8
"""
Legacy NewsPilot runtime (pre-LangGraph).
...
from NewsPilot import __version__
from NewsPilot.core import load_config
from NewsPilot.core.analyzer import convert_keyword_stats_to_platform_stats
from NewsPilot.crawler import DataFetcher
from NewsPilot.storage import convert_crawl_results_to_news_data
from NewsPilot.utils.env import env_flag
from NewsPilot.utils.time import is_within_days
...
class NewsAnalyzer:
    ...
    def __init__(self):
        print("正在加载配置...")
        config = load_config()
        print(f"NewsPilot v{__version__} 配置加载完成")
    ...
    def _should_open_browser(self) -> bool:
        # MCP/服务端触发时通常不应打开浏览器
        if not env_flag("NP_OPEN_BROWSER", default=True):
            return False
    ...
    def _crawl_rss_data(self):
        ...
        try:
            from NewsPilot.crawler.rss import RSSFetcher, RSSFeedConfig
